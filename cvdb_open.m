classdef cvdb < handle
    ...
    methods(Access=public)
        function this = cvdb(file_name)
            this.connh = [];
            if nargin < 1
                file_name = 'cvdb.cfg';
            end
                
            if exist('cvdb.cfg','file')
                fid = fopen('cvdb.cfg');
        ...
        function conn = open(varargin)
            this.cfg = helpers.vl_argparse(this.cfg,varargin{:});
            try
                conn = dbconn2(server_name,name,user,pass);
            catch exception
    ...
    methods(Access=private)
        function conn = dbconn2(server_name, name, user, pass)
