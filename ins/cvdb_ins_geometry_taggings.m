function [] = cvdb_ins_geometry_taggings(conn, ...
                                         tag_list, primitives_key_list)
    connh = conn.Handle;
    ...
    for i = 1:length(tag_list)
        ...
        if (rs2.next())
            tag_id = rs2.getInt(1); 
            for j = 1:length(primitives_key_list)
                stm2 = connh.prepareStatement(['INSERT INTO geometry_taggings ' ...
                                    '(primitives_id, tag_id) ' ...
                                    'VALUES(?, ?)']);
                stm2.setInt(1, primitives_key_list(j));
