function [stereo_set] = cvdb_select_stereo_set(conn, set_name)
    ...
    if (count > 0)    
        ...
        while (rs.next() && rs2.next())
            row_num = row_num+1
            stereo_set(row_num).img1 = char(rs.getString(1));
            stereo_set(row_num).img2 = char(rs2.getString(1));
        end
    end