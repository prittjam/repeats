...
%
function [res,gt,cam] = stability(name_list,solver_list,all_solver_names,varargin)
    repeats_init;
    ...
    res = cell2table(cell(0,10), 'VariableNames', ...
                     {'solver','ex_num','rewarp',...
                      'ransac_rewarp','q','ransac_q', ...
                      'num_sol','num_real','num_feasible', 'alg_err'});
            ...
            for ccd_sigma = cfg.ccdsigmalist
                for k = 1:numel(solver_list)
                    ...
                    for k2 = 1:samples_drawn
                        ...
                        if ~isempty(M)
                            [~,opt_warp_list(k2),mdl_best_ind] = ...
                                calc_opt_warp(truth,cam,M,P,wplane,hplane);
                            optq_list(k2) = ...
                                calc_opt_q(truth,cam,M,P,wplane, ...
                                           hplane);
                            alg_err(k2) = ...
                                solver_list(k).calc_mean_alg_err(xdn,idx,cc,M(mdl_best_ind));
                        else
...
function [optq,opt_warp,best_ind] = calc_opt_warp(gt,cam,M,P,w,h)    
