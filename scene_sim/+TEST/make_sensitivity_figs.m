...
solver_list_str = string(solver_list);
Lia = res.sigma > 0;
Lib = ismember(string(res.solver), ...
               setdiff(solver_list_str, ...
                       {'$\mH22\vl s_i$','$\mH22\vl$','$\mH2\vl\vu$'}));
Lid = ismember(string(res.solver), ...
               setdiff(solver_list_str, ...
                       {'$\mH22\lambda$','$\mH22\lambda_1\lambda_2$'}));
...
is_valid = Lia;
boxplot_colors = make_boxplot_colors(res(is_valid,:),colormap); 
ax7 = make_grouped_boxplot(res(is_valid,:), {'ransac_xfer'},{ 'sigma', 'solver'}, ...
...
drawnow;
cleanfigure;
matlab2tikz([target_path sprintf('ransac_xfer_sensitivity_%s.tikz',rigid_xform)], ...
            'width','\fwidth','extraAxisOptions',axis_options);
